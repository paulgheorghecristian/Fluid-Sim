...
Shader::Shader(const std::string& vShaderPath, const std::string& fShaderPath)
{
    ...
    std::string vertex_shader_source = load_shader(vShaderPath);
    std::string fragment_shader_source = load_shader(fShaderPath);
...
void Shader::loadGeneralUniform (const std::string &name, int data) {
    glUniform1i (glGetUniformLocation(program, name.c_str ()), data);
}

void Shader::loadGeneralUniform (const std::string &name, float data) {
